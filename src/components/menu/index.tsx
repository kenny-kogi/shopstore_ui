        ...
        <ul
          className={`${
            submenuItems && isOpenSubmenu
              ? "absolute opacity-100 right-0 w-56 p-2 mt-2 space-y-2 text-gray-600 bg-white border border-gray-100 rounded-md shadow-md"
              : "opacity-0 h-0 overflow-hidden absolute right-0"
          } transition-opacity duration-200`}
          aria-label="submenu"
        >
          {submenuItems.map((item: ISubmenuItem) => (
            <li className="flex" key={item.text}>
              <Link legacyBehavior href={item.routeHref}>
                {item.icon ? (
